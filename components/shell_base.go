...
func (e ShellBase) Body(ctx context.Context) Node {
	...
	return Body(Class("hide-right font-sans"), Attr("x-data", `{ theme: localStorage.getItem('theme') || 'light' }`), Attr(":data-theme", "theme"), Attr("hx-ext", "preload, morph"), Attr("hx-boost", "true"), Attr("hx-indicator", "#global-loading-indicator"), Attr("hx-push-url", "true"),
