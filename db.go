...
func initDBWith(db *gorm.DB, config LarivConfig, migrations *registry.ImmutableRegistry[UsefulFilesystem], hooks *registry.ImmutableRegistry[DBInitHook]) error {
	sqlDB, err := db.DB()
	if err != nil {
	...
	if err := gooseUpPluginMigrations(context.Background(), sqlDB, config, migrations); err != nil {
		return fmt.Errorf("goose migrations: %w", err)
	}

	for _, p := range *hooks.AllStable() {
		db = p.Value(db)
	}
	return nil
}
