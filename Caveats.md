...
- **Detail / read-only (string label):** Use `registry.PairValueFromKey(getters.Key[string]("$in.FieldName"), MyFieldChoices)` with `components.FieldText` (or any `Getter[string]` that should show the **Value** from the slice). Empty stored key → `""`; known key → `Value`; unknown key → the raw key (same display fallback as selects). Do not reimplement that mapping in a one-off getter.
- **Forms and filters (`InputSelect[string]`):** `Choices: getters.Static(MyFieldChoices)`. For **`Getter`**, use **`registry.PairFromGetter`**: pass the same choices slice and a `getters.Key[string]` to the current stored key—**`$in.FieldName`** on create/update/detail form contexts, **`$get.FieldName`** on list/selection filter forms. It returns a `getters.Getter[registry.Pair[string, string]]`: empty key → zero pair; known key → pair from the slice; unknown legacy key → `{Key: s, Value: s}`. If the form must show a **specific default option when the stored value is empty** (e.g. a create flow that pre-selects "Enrolled" or "Created"), keep a small custom getter: `PairFromGetter` is not appropriate because it does not map empty → a chosen default. Examples: `academicrecords` create status, `assignmentsubmissions` submission status, `studentpayments` create payment method.
...
- **Helpers** (in `registry/registry.go`): `registry.PairFromPairs`, `registry.PairValueFromKey` (read-only label string), `registry.PairFromGetter` (`InputSelect` current pair), `registry.MapFromPairs`, and (for legacy or map-based code paths) `registry.PairFromMap` / `registry.PairsFromMap`. **`registry.KeysFromPairs`** returns keys in slice order (e.g. generators or tests). **`registry.PairGetter`** is a different API (key + `Getter[map[K]V]`, strict map lookup); it does not replace `PairFromGetter` for choice slices.
