...
func announcementsListSemesterEnvQueryPatcher(_ *baseviews.View, r *http.Request, query *gorm.DB) *gorm.DB {
	...
	sub := query.Session(&gorm.Session{NewDB: true}).Model(&AnnouncementSemesterDetails{}).
...
func createAnnouncementWithSemester(successURL getters.Getter[string]) func(*baseviews.View) http.Handler {
	return func(view *baseviews.View) http.Handler {
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			record := new(p_announcements.Announcement)
			err = db.Transaction(func(tx *gorm.DB) error {
				...
				return UpsertAnnouncementSemester(tx, record.ID, semesterID)
			})
...
func updateAnnouncementWithSemester(successURL getters.Getter[string]) func(*baseviews.View) http.Handler {
	return func(view *baseviews.View) http.Handler {
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			values, fieldErrors, err := view.ParseForm(w, r)
			if err != nil {
			...
			err = db.Transaction(func(tx *gorm.DB) error {
				query := tx.Model(new(p_announcements.Announcement)).Where("id = ?", id)
				...
				if len(regularValues) > 0 {
					updateQuery := tx.Model(new(p_announcements.Announcement)).Where("id = ?", id)
					for _, queryPatcher := range view.QueryPatchers {
				...
				return UpsertAnnouncementSemester(tx, id, semesterID)
			})
			if err != nil {
				fieldErrors["_form"] = err
				view.RenderWithErrors(w, r, fieldErrors, values)
...
func patchAnnouncementViews() {
	successURL := lago.GetterRoutePath("announcements.DetailRoute", map[string]getters.Getter[any]{
		"id": getters.GetterAny(getters.GetterKey[uint]("$id")),
	})

	lago.RegistryView.Patch("announcements.ListView", func(v *baseviews.View) *baseviews.View {
		return v.WithQueryPatcher("announcements_semesters.filter_env_semester", announcementsListSemesterEnvQueryPatcher)
	})

	lago.RegistryView.Patch("announcements.CreateView", func(v *baseviews.View) *baseviews.View {
		v.Handlers[http.MethodPost] = createAnnouncementWithSemester(successURL)
		return v
	})

	lago.RegistryView.Patch("announcements.UpdateView", func(v *baseviews.View) *baseviews.View {
		v.Handlers[http.MethodPost] = updateAnnouncementWithSemester(successURL)
		return v
	})
}
